...
use anthropic_ai_sdk::types::message_batches::{
    DeleteMessageBatchParams, MessageBatchClient, MessageBatchError,
};
use std::env;
...
async fn main() {
    ...
    match client
        .delete_message_batch(&DeleteMessageBatchParams::new("msgbatch_batch_id"))
        .await
    {
        Ok(results) => {
            info!("Successfully deleted message batch: {:?}", results);
        }
        Err(e) => {
            error!("Error: {}", e);
        }
